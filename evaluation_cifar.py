...
item = datasets.CIFAR10(root='./datasets/cifar10', train=False, transform=transform_chain, download=True)
test_dataloader = data.DataLoader(item, batch_size=100, shuffle=False, num_workers=2)


def main(config):
    model = Classifier(10, classifier_name=config.architecture, dataset="cifar10", pretrained=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='cifar10_resnet20_ours',
                         help='classifier path')
    parser.add_argument('--architecture', type=str, default='resnet20',
